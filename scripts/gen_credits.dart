...
void main() async {
  ...
  final gitDeps = gitDepsList.map((d) {
    return MapEntry(
      d.key,
      join(
  ...
  final gitPubspecs = await Future.wait(
    gitDeps.map((d) {
      ...
      return get(Uri.parse(d.value))
          .then(parser)
          .catchError(
            (_) => get(
              Uri.parse(d.value.replaceFirst('/main', '/master')),
            ).then(parser),
          );
    }),
  );
  ...
  print(
    packageInfo
        .map(
          (package) =>
              '<li>\n  <a href="${package.latestPubspec.homepage ?? package.url}">${package.name}</a> - ${package.description.replaceAll('\n', '')}\n</li>',
        )
        .join('\n'),
  );
  print(
        ...
        .map((package) {
          final packageUrl =
              ...
              gitDepsList
                  ...
                  .url
                  .toString();
          return '<li>\n  <a href="$packageUrl">${package.name}</a> - ${package.description?.replaceAll('\n', '')}\n</li>';
        })
        .join('\n'),
  );
  exit(0);
