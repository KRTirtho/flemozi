...
pub struct EmoticonTab {
    pub entries: Vec<EmoticonEntry>,
...
impl EmoticonTab {
    pub fn new() -> Self {
        let entries: Vec<EmoticonEntry> = EMOTICONS
            .iter()
            .map(|t| EmoticonEntry {
                text: t.text,
                category: t.category,
        ...
        Self {
            ...
            selected: 0,
            visible: HashSet::new(),
            scroll_id: "emoticon-scroll".into(),
            saved_search: SearchState::new(),
        }
    }

    pub fn apply_search(&mut self, query: &str) {
        self.filtered = filter_emoticons(&self.entries, query);
